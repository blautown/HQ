...
| Client-brief handoff | Filled-in `client-brief.md`, one per seat | Claude Code drafted; **Founder** sends | `binding001-jrba/client-brief/` | **Drafted, ready** — founder still needs to actually send both |
| Baseline drift check | `dk0/drift-check.md` run against both seats | Claude Code drafted the per-seat script; **Founder** runs it in JRBA's live project | `binding001-jrba/agents/<name>/drift-check.md` | **Drafted, ready** — founder still needs to run both and report the verdict back |
